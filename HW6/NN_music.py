...
for i in range(numTrainData):
	trainDataArray[i,0] = normalize(trainDataList[i][0], yearMax, yearMin)
	trainDataArray[i,1] = normalize(trainDataList[i][1], recordLMax, recordLMin)
	trainDataArray[i,2] = normalize(trainDataList[i][2], binaryMax, binaryMin)
	trainDataArray[i,3] = normalize(trainDataList[i][3], binaryMax, binaryMin)

for i in range(numTrainData):
		if trainKeyList[i][0] == 'yes':
			trainKeyArray[i][0] = 1

for i in range(numDevData):
	devDataArray[i,0] = normalize(devDataList[i][0], yearMax, yearMin)
	devDataArray[i,1] = normalize(devDataList[i][1], recordLMax, recordLMin)
	devDataArray[i,2] = normalize(devDataList[i][2], binaryMax, binaryMin)
	devDataArray[i,3] = normalize(devDataList[i][3], binaryMax, binaryMin)
...
for i in range(devDataArray.shape[0]):
	...
	finalLayerInput = np.dot(devWeight2Array_GD,hiddenLayerOutput)
	finalLayerOutput = sigmoid(finalLayerInput)
	#print(finalLayerOutput[0,0])
	finalLayerOutInt = finalLayerOutput[0,0]
	if finalLayerOutInt < 0.5:
		print("no")
	else:
		print("yes")